...
// Draw commands
const Polygon = createToken({ name: 'Polygon', pattern: /polygon\b/ });
...
class AstBuilder extends BaseCstVisitor {
    ...
    topLevelStatement(ctx) {
        if (ctx.Semicolon) return null;
        if (ctx.uniformDeclaration) return this.visit(ctx.uniformDeclaration);
        if (ctx.functionOrVariableDeclaration) return this.visit(ctx.functionOrVariableDeclaration);
        if (ctx.formattingDirective) return this.visit(ctx.formattingDirective);
        if (ctx.drawCall) return this.visit(ctx.drawCall);
        return null;
    }
    ...
    typeSpec(ctx) {
        let baseType;
        if (ctx.FloatType) baseType = 'float';
        else if (ctx.IntType) baseType = 'int';
        else if (ctx.Vec2Type) baseType = 'vec2';
        else if (ctx.Vec3Type) baseType = 'vec3';
        else if (ctx.AnyType) baseType = 'any';
    ...
    drawCall(ctx) {
        ...
        return {
            ...
            command: 'polygon',
            arguments: args,
            line: ctx.Polygon[0].startLine,
            column: ctx.Polygon[0].startColumn
        };
    }
    ...
    comparisonExpression(ctx) {
        return this.buildBinaryExpression(
            ...
            ctx.LessThan || ctx.GreaterThan || ctx.Not,
            (op) => op.image
        );
    }
    ...
    multiplicativeExpression(ctx) {
        return this.buildBinaryExpression(
            ctx.unaryExpression,
            ctx.Multiply || ctx.Divide || ctx.Modulo,
            (op) => op.image
        );
    }
    ...
    primaryExpression(ctx) {
        if (ctx.forExpression) return this.visit(ctx.forExpression);
        if (ctx.switchExpression) return this.visit(ctx.switchExpression);
        if (ctx.Number) {
            return {
        ...
        if (ctx.parenthesizedOrVector) return this.visit(ctx.parenthesizedOrVector);
        if (ctx.listOrRange) return this.visit(ctx.listOrRange);
